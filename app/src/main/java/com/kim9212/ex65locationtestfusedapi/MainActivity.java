package com.kim9212.ex65locationtestfusedapi;
...
public class MainActivity extends AppCompatActivity {
    ...
    /// Google Fused Location 라이브러리 : 적절한 위치정보제공자를 선정하여 위치정보를 제공 ////

    // 외부 라이브러리 추가 : play-services 라이브러리 검색, 근데 너무 무거워서.. 명시적으로 -location버전만 받기...
    // 디바이스에 Google Play Store앱이 없으면 실행 안됨
    ...
    public void clickBtn(View view) {
        ...
        //준비가 되었으니 위치정보 취득을 위한 연결 시도!!
        googleApiClient.connect();//연결이 성공하면 connectionCallbacks리스너 객체의 onConnect()메소드 실행
    ...
    GoogleApiClient.ConnectionCallbacks connectionCallbacks= new GoogleApiClient.ConnectionCallbacks() {
        ...
        public void onConnected(@Nullable Bundle bundle) {
            //연결에 성공했을때..(아직 위치정보를 얻은 것은 아님..얻을 수 있는 상태)
    ...
    //액티비타가 화면에 보이지 않으면 위치정보를 더 이상 갱신하지 않도록
    @Override
